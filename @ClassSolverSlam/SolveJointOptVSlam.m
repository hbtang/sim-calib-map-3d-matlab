function SolveJointOptVSlam(this, measure, calib, map, options)
...
[q,resnorm,residual,exitflag,output,lambda,jacobian] = lsqnonlin(...
    @(x)CostJointOptVSlam(x, mk, odo, time, calib, this.setting, options), q, [], [], options_optim);
...
function [ vecCost, matJacobian ] = CostJointOptVSlam( q, mk, odo, time, calib, setting, options )
%COST

% error configure
stdErrRatioOdoLin = max(setting.error.odo.stdratio_lin, 1e-3);
stdErrRatioOdoRot = max(setting.error.odo.stdratio_rot, 1e-3);
stdErrMinOdoLin = setting.error.odo.stdmin_lin;
stdErrMinOdoRot = setting.error.odo.stdmin_rot;
...
for i = 1:odo.num
    ...
    std_trans = max(norm(ps2d_b1_b2_odo(1:2))*stdErrRatioOdoLin, stdErrMinOdoLin);
    std_rot = max(abs(ps2d_b1_b2_odo(3))*stdErrRatioOdoRot, stdErrMinOdoRot);
